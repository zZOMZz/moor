...
import { SEARCH_LIMITS, searchEntrySchema, type SearchEntry } from '@moor/protocol/search-protocol';

export type SearchDocument = { entries: SearchEntry[]; partial: boolean };
export type SearchSnapshotFile = {
  path: string;
  size?: number;
...
export type SearchSnapshotChange = {
  path: string;
  previousPath?: string;
  kind: 'added' | 'deleted' | 'modified' | 'renamed';
  before: SearchSnapshotFile | null;
  after: SearchSnapshotFile | null;
};
export type SearchSnapshotDiff = {
  version: 1;
  basis: 'project-snapshot';
  changes: SearchSnapshotChange[];
  partial: boolean;
...
export type FrozenSearchDiff = {
  ...
  turnId: string;
  itemIndex: number;
  diff: Readonly<SearchSnapshotDiff>;
};
