...
export function decodePushKey(value: string, length: number) {
...
export function validatePushSubscription(input: unknown): MoorPushSubscription {
  ...
  const key = decodePushKey(parsed.keys.p256dh, 65);
  ...
  decodePushKey(parsed.keys.auth, 16);
...
export class RelayNotifications {
  ...
  private validateSubscription: (input: unknown) => MoorPushSubscription;
  ...
  constructor(
    ...
    options: {
      ...
      validateSubscription?: (input: unknown) => MoorPushSubscription;
    ...
    this.transport =
      options.transport ??
      ({
        state: { configured: false, reason: '此入口未配置推送服务' },
        async send() {
          return { status: 'failed' };
        },
      } satisfies WebPushTransport);
    this.validateSubscription = options.validateSubscription ?? validatePushSubscription;
  ...
  subscribe(owner: string, loginHash: string, input: unknown): PushSubscriptionView {
    ...
    const parsed = pushSubscriptionRequestSchema.parse(input),
      subscription = this.validateSubscription(parsed.subscription),
