...
import { generateDeviceEncryptionKey, seal } from '@moor/e2ee/e2ee-crypto';
import {
  generateTrustRoot,
  encryptionKeyId,
  signTrustManifest,
...
test('attachment writes and original recovery require the authenticated recovery capability before any frame, including legacy and historical mappings', async (t) => {
  const { ATTACHMENT_OPERATIONS_FEATURE } = await import('@moor/protocol/session-control-protocol');
  for (const version of [1, 2])
    for (const supported of [false, true]) {
      const f = await fixture(t),
