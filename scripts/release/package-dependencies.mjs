...
import { cp, open, readFile, readdir, realpath, stat } from 'node:fs/promises';
...
export async function workspaceManifestDirectories(repository = process.cwd()) {
    ...
    for (const entry of await readdir(base, { withFileTypes: true })) {
      ...
      try {
        ...
        if (typeof manifest.name !== 'string' || !manifest.name.startsWith('@moor/'))
          throw new Error('Invalid Moor workspace manifest');
