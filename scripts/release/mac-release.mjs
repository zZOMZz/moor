...
const required = [
  ...
  `${programRoot}/main/main.cjs`,
  `${programRoot}/preload/preload.cjs`,
  `${programRoot}/settings/settings.html`,
...
function programPath(path, directory) {
  ...
  if (segments[1] === 'Resources' && segments.length >= 3) {
    ...
    if (segments[2] === 'app' && segments.length >= 4) {
      ...
      const desktopPath = segments.slice(3).join('/');
      const desktopDirectory = ['main', 'preload', 'settings'].includes(entry);
      ensure(
        desktopFiles.has(desktopPath) ||
          (desktopDirectory && segments.length === 4) ||
          ['licenses', 'runtime'].includes(entry),
        'Unexpected Moor program entry',
      );
      if (desktopFiles.has(desktopPath))
        ensure(!directory, 'Unexpected desktop resource directory');
      if (desktopDirectory && segments.length === 4)
        ensure(directory, 'Unexpected desktop resource file');
