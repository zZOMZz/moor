...
const help = `Usage: node scripts/release/deploy-relay.mjs <check|deploy> [options]
...
try {
  ...
  const script = await readFile(join(root, 'scripts/release/deploy-relay-remote.sh'), 'utf8');
  ...
  if (mode === 'deploy') {
    ...
    run(process.execPath, ['scripts/release/package.mjs', 'relay']);
