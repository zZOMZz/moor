...
正文中的 `items` 保存文本、思考、工具调用、审批信息和系统通知。Mirror 按内容项的运行时值推导容器，其中的普通文本字段按字符串值更新；回合身份与生命周期使用显式声明的 CRDT 字段。具体合并粒度见 [Loro 与 CRDT 学习文档](loro-crdt.md)。界面只渲染支持的内容，Agent 输出中的 HTML 不执行，也不自动加载远程图片。
