...
Agent 版本测试使用独立旧版 SQLite、合成启动参数和确定性信号，覆盖迁移、原生映射、确认事务回滚、等待快照期间切换版本及 Fork 恢复。能力检查另覆盖实际目录替换和中转登录/连接变化；真实 app 测试覆盖旧版本退出目录后的发送、缺失配置的历史读取、离线缓存与原请求确认。测试不会运行参数中描述的程序或访问真实 Agent 账户。固定范围与未完成的配置入口见[会话固定 Agent 版本](runtime.md#会话固定-agent-版本)。
