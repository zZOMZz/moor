...
浏览器 `startup.js` 的 `__ENTRY__` 通过 `paths` 映射回 `src/web/entry.ts`，对应构建脚本替换后的真实调用。原生 GUI 验证会先把 `createSecureDesktopHost` 打包到临时路径再动态导入，该函数用带有调用方说明的 `@public` 标记保留。新增动态加载入口时应补充真实引用关系；只有无法静态解析的具体入口才使用这种标记，不要忽略整个目录或关闭导出检查。
...
| Agent 本机设置与角色兼容 | [本机设置](../src/runtime/agent-settings.ts)、[固定版本](../src/runtime/session-agent.ts)、[角色主机](../src/runtime/session-roles.ts)、[旧角色数据](../src/web/roles.ts) | [本机 CLI/IPC](../tests/agent-settings-host.test.ts)、[角色主机](../tests/session-roles.test.ts)、[角色中转](../tests/roles-relay.test.ts)、[旧工作区记录](../tests/workspace-feature-compatibility.test.ts) |
