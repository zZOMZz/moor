...
[Knip 配置](../knip.jsonc) 包含生产入口、构建脚本、合成测试和原生 GUI 验证入口。默认检查保留测试引用，并启用 `ignoreExportsUsedInFile`，让名单聚焦于定义本身未使用的代码；仅在同一文件内使用的 schema、组件或辅助函数仍需保留。若要收窄这些定义的导出范围，可另行移除多余的 `export`，不能直接删除定义。该选项的行为见 [Knip 配置说明](https://knip.dev/reference/configuration#ignoreexportsusedinfile)。

未使用导出和导出类型的规则已设为 `error`。CI 运行默认的 `corepack pnpm knip`，出现新增问题时会以非零状态退出并阻止检查通过。`knip:production` 用于单独审查生产引用，其结果仍需结合测试接口和动态加载入口判断。

两条 Knip 命令每次重新分析，不启用 `--cache`。当前锁定版本的文件分析缓存不会因 `paths` 或 `ignoreExportsUsedInFile` 改动而全部失效，复用旧缓存会漏掉新引用或重新报告文件内使用的定义。

浏览器 `startup.js` 的 `__ENTRY__` 通过 `paths` 映射回 `apps/web/src/app/entry.ts`，对应构建脚本替换后的真实调用。原生 GUI 验证会先把 `createSecureDesktopHost` 打包到临时路径再动态导入，该函数用带有调用方说明的 `@public` 标记保留。新增动态加载入口时应补充真实引用关系；只有无法静态解析的具体入口才使用这种标记，不要忽略整个目录或关闭导出检查。

清理时按名单中的文件和符号定位，检查初始化副作用、动态加载和同名定义。未使用的类型转导出只删除对应 `export type { ... }`，保留仍有引用的原始类型与本地导入。`knip:production` 排除测试，适合检查生产可达性，其中仅被测试使用的代码不能直接作为全仓库删除名单。清理后仍需运行类型、测试、构建和格式四项检查。
...
| Agent 本机设置与角色兼容 | [本机设置](../packages/host/src/agents/settings.ts)、[固定版本](../packages/host/src/sessions/agent.ts)、[角色主机](../packages/host/src/sessions/roles.ts)、[旧角色数据](../apps/web/src/features/roles/roles.ts)                     | [本机 CLI/IPC](../tests/integration/agent-settings-host.test.ts)、[角色主机](../tests/integration/session-roles.test.ts)、[角色中转](../tests/integration/roles-relay.test.ts)、[旧工作区记录](../tests/integration/workspace-feature-compatibility.test.ts) |
| 导航、缓存与输出阅读     | [Web 应用](../apps/web/src/app/app.ts)、[缓存](../apps/web/src/platform/cache.ts)、[内容展示](../apps/web/src/components/content.ts)                                                                                                   | [web](../tests/integration/web.test.ts)、[ui](../tests/integration/ui.test.ts)、[startup](../tests/integration/startup.test.ts)                                                                                                                              |
...
| 进程恢复与主机独占       | [恢复控制](../apps/desktop/src/main/recovery.cjs)、[所有权锁](../packages/e2ee/src/node/exclusive-lock.ts)                                                                                                                             | [recovery](../tests/integration/recovery.test.ts)、[runtime-lock](../tests/integration/runtime-lock.test.ts)                                                                                                                                                 |
