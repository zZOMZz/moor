...
电脑显示名称由执行主机在运行数据库中保存，携带独立递增版本。桌面连接设置的“保存电脑名称”只通过私有 IPC 修改这项元数据；整页保存若仅改名也不会重启执行组件。初次升级以已有本机设置初始化，后续重启不再用旧设置覆盖主机保存的名称。独立主机显式指定 `--name` 时可更新名称，设备身份和会话目标不变。

名称在本机保存与远端同步分别确认。v3 主机连接及重连发布已保存的名称版本，中转事务更新设备记录和目录后确认，并通知在线访问端刷新；旧版本不能覆盖新名称。中转离线时显示等待确认，旧中转未报告名称确认时显示不支持同步，不会误报全端成功。v4 名称只出现在经过认证的加密目录中；访问端在当前连接内拒绝名称版本回退、同版本不同内容及已报告名称的移除。恢复连接只同步名称状态，不重放用户指令。名称与模型基础修复已在[客户端改进第一阶段](client-unification.md#阶段与提交边界)完成；单一客户端与主界面整理仍按后续阶段实施。
...
刷新模型选项时，主机会打开一个临时 AgentSession，读取它报告的能力后关闭；已有会话使用固定 Agent 版本和实际执行目录，新会话使用已登记项目。支持 `agent-model-options-v1` 的主机允许在临时会话中检查指定模型切换后的选项，整个探测不发送用户 prompt，也不批准权限请求。项目目录、程序文件、会话归属或请求连接变化后，不发布迟到的能力结果。

模型能力观测按主机身份、Agent 版本、项目和会话隔离，并包含执行目录及程序文件指纹与读取时间。程序原地升级或目录替换后，旧观测不能再作为该范围的能力返回。公共全局目录不再把设置页或另一项目的模型缓存冒充为当前项目选项；完整程序路径保留在执行主机。普通客户端连接支持新版能力协议的主机后，即使已有缓存，也在首次进入目标时探测；模型选择器打开及选择模型时重新检查，保留草稿中的过期选择并提示重新选择。

当前模型与新会话初始默认模型分别保存。临时探测得到的新会话默认值不能解释为既有原生会话的当前模型；既有会话留空时，界面标明发送时确认。执行时使用本次实际打开或恢复的 Agent 配置：先应用所选模型，消费返回的完整配置，再校验该模型的 effort。后续 ACP 配置通知替换旧观测，移除的选项不会沿用。已知的 Codex 程序版本不兼容与 ChatGPT 账号不支持模型使用固定错误说明，不公开原始诊断，也不自动换模型或重发。留空保持 Agent 当前设置，不代表重置或降低权限。

## 会话固定 Agent 版本

加密界面与普通界面共用模型控件。加密模型草稿按完整账号、设备、项目、会话和目录映射保存，使用版本比较拒绝另一页面的覆盖；发送前重新检查草稿和当前回合，再把选项固定进原操作。旧型号会保留并显示不可用，断线、重连和恢复草稿均不发送指令。模型能力自动读取不会覆盖待处理审批或其他需要用户行动的提示。
