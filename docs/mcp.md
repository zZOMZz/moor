...
在菜单的“加密工作区”中，先连接并打开已确认产品副本中的会话，再点击“额外 MCP”。打开面板不读取主机；“读取项目允许的 MCP”才通过有限加密 IPC 取得当前目录。目录只含名称、说明、不透明版本编号和传输类型，不包含连接地址、程序路径、参数或凭据，也不启动 Agent 或服务。

保存选择会生成独立的审阅编号，绑定账号、根、客户端设备、执行主机、运行工作区、项目、会话及完整产品映射。本机草稿通过 Web Lock 和条件写入协调多个页面；另页已修改的选择需要重新读取、审阅。离线只能保留或移除已保存的完整原版本，不能根据旧目录添加服务。关闭面板、断线或切换目标会清除内存目录，重新连接仍需手动读取。

发送按钮固定本次实际显示的目标、附件和 MCP 选择。Moor 在上传附件前核对目录，在最终暂存指令时再次核对，任何缺失或变化都拒绝发送，不静默替换版本。完整审阅与指令正文、原操作编号一起保存在本机原操作账本中，并纳入请求摘要；保存原操作不启动服务，随后才由用户这次明确发送交给主机。

结果未知时，原选择随原请求固定，刷新、重连和恢复页面都不会重放。核查或重试使用原编号、正文和目标；主机接受后，本机只消费同一个审阅编号，新保存的草稿仍保留。封存未确认时继续阻断新指令；已确认未执行的原操作不把草稿变成自动执行队列。没有新选择的下一回合不附加上一回合的 Moor MCP。

MCP 目录读取要求 `session-mcp-v1`；加密发送选中服务还要求已认证的主机工作区报告 `secure-turn-authority-v1`。旧主机仍可提供支持的只读目录，本机选择可以保留，但不能省略新授权边界发送。新主机把一次 RPC 的映射租约与整个回合的授权检查分开：回包完成不撤销仍有效的回合；原客户端断开、信任或产品映射变化、运行身份变化则使相应授权失效，主机请求停止和关闭对应 Agent。其他仍有效客户端的回合不因一个客户端断开而失去授权。
...
显式 `secure send --mcp-server-ids ...` 要求 `secure-turn-authority-v1`，并在收到主机接受回执后保持原授权连接，前台只读等待该原操作绑定的精确回合结束。安全目录审阅与用户回合编号先进入原操作记录，接受回执先保存，等待不重新发送指令。默认不设等待时限；可用 `--timeout MS` 明确设置 1–86400000 毫秒。成功输出另外包含该回合的 `execution` 结果，不把接受回执等同于执行完成。

Ctrl-C、明确时限到期、连接失效或无法核对精确回合时，CLI 保留已接受的原记录并关闭授权连接，主机请求撤销相应回合；已派发的外部动作仍需核查。等待失败不会把接受状态改回待发送，也不自动重试。带完整审阅和精确回合绑定的待确认原操作，手动 `secure retry` 取得接受回执后沿用同样的等待流程。新普通文字与 MCP 发送都保存精确的 `userTurnId`；升级前缺少此字段的旧待确认指令，包括普通文字指令，只能 `secure inspect` 或 `secure abandon`，不能猜测最新回合后直接重试。已接受记录可读取原回执，不重新执行。
...
可信桌面新增的定向验证覆盖安全目录、完整身份草稿、跨页竞争、冻结审阅、原回执丢失后的同请求恢复、后续回合不继承，以及面板关闭或连接变化时拒绝迟到读取和写入。完整安装包、部署与设备结果以[验收记录](validation.md)中对应批次为准；功能接通不等于真实服务或多设备验收完成。
