...
import type { AttachmentAction } from '@moor/protocol/attachment-protocol';
import type { AttachmentReference } from '@moor/protocol/content-protocol';
import { attachmentDraftKey } from '../../apps/web/src/features/attachments/attachments';
...
test('real app preserves attachment-only new drafts and scoped manual delivery across reloads, offline states and target switches', async () => {
  ...
  await build({
    entryPoints: ['apps/web/src/app/app.ts'],
    outfile,
    bundle: true,
    platform: 'node',
    format: 'esm',
    ...
    plugins: [
      {
        ...
        setup(builder) {
          ...
          builder.onLoad({ filter: /.*/, namespace: 'synthetic' }, () => ({
            loader: 'js',
            contents: `export const read=async key=>structuredClone(globalThis.__moorAttachmentAppCache.get(key)); export const write=async (key,value)=>{globalThis.__moorAttachmentAppCache.set(key,structuredClone(value));}; export const compareAndSet=async(key,expected,value)=>{const cache=globalThis.__moorAppCache;if(JSON.stringify(cache.get(key))!==JSON.stringify(expected))return false;cache.set(key,structuredClone(value));return true;}; export const compareTaskSubmission=async()=>{throw new Error("unexpected task submission");}; export const compareText=async(key,expected,value,current,signal)=>{if(signal?.aborted||!current())throw new Error("stale draft");const cache=globalThis.__moorAppCache;if(cache.get(key)!==expected)return false;cache.set(key,value);return true;}; export const compareWrite=async(key,expectedRevision,value,current)=>{if(!current())throw new Error("stale Git target");const cache=globalThis.__moorAttachmentAppCache;if((cache.get(key)?.cacheRevision??0)!==expectedRevision)return false;cache.set(key,structuredClone(value));return true;}; export const compareDraftBundle=async()=>{throw new Error("unexpected role draft application");}; export const clear=async()=>globalThis.__moorAttachmentAppCache.clear();`,
          }));
          builder.onLoad({ filter: /\/apps\/web\/src\/app\/app\.ts$/ }, async (args) => ({
            loader: 'ts',
            resolveDir: resolve('apps/web/src/app'),
            contents:
              (await readFile(args.path, 'utf8')) +
              '\nexport { loadDevices, loadSessions, selectDevice, openSession, currentAttachments, sendTurn, addAttachments, openHistoryAttachment }; export { disposeUI } from "../components/ui";\n',
          }));
        },
      },
    ],
