...
import type { Navigation, SessionActionKind } from '../../apps/web/src/components/ui';
...
test('session menus, host-confirmed rename, archive filters and long lists work inside the mobile drawer', async () => {
  ...
  const { showShell, showNavigation, disposeUI } = await import('../../apps/web/src/components/ui');
  const space = {
    id: 'w',
