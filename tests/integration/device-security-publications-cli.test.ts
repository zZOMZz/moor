...
} from '../../apps/cli/src/security/commands';
import type { DeviceManager, DevicePairingReceipt } from '@moor/e2ee/node/device-manager';
import type { PublicTrustEntry } from '@moor/e2ee/trust-publication';
import { CLI_GOOGLE_FLOW_KEY } from '../../apps/cli/src/google-auth';
...
async function fixture(t: TestContext, options: { googleLogin?: boolean } = {}) {
  ...
  return {
    ...
    run: (input: unknown, path = file) =>
      child(
        packagedEntry ?? resolve('apps/cli/src/security/main.ts'),
        ['--data-file', path],
    ...
    cli: (args: string[], input = '') =>
      child(
        cliEntry ?? resolve('apps/cli/src/main.ts'),
