...
} from '@moor/host/agents/acp/driver';
import type {
  AgentCallbacks,
  AgentMcpServer,
  AgentOpenOptions,
  AgentRunBinding,
} from '@moor/host/agents/driver';
import { AppError } from '@moor/protocol/protocol';
import { normalizeAgentContent, normalizeAgentToolContent } from '@moor/host/agents/attachments';
...
function fixture(
  ...
  const driver = createAcpDriver((command, args, options) => {
    ...
    const child = spawn(
      process.execPath,
      [resolve('tests/fixtures/synthetic-acp-mcp-config.mjs'), JSON.stringify(settings)],
