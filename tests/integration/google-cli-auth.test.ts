...
import {
  ...
  CLI_GOOGLE_FAILED,
  CLI_GOOGLE_FLOW_KEY,
} from '../../apps/cli/src/google-auth';
import { Store, token } from '@moor/gateway/accounts';
import { createApp } from '@moor/gateway/http';
import type { GoogleIdentity, GoogleOidcProvider } from '../../apps/relay/src/google-oidc';
...
for (const invalid of ['wrong-account', 'local-actor', 'too-many-features'] as const)
  test(`Google CLI rejects ${invalid} identity without installing or reissuing credentials`, async (t) => {
    const f = await fixture(t),
      { input } = await f.ready();
    f.intercept(async (url, init) => {
      if (!url.endsWith('/api/me')) return;
      const response = await fetch(url, init),
        value = await response.json();
      if (invalid === 'wrong-account') value.actor.accountId = 'another-account';
      else if (invalid === 'local-actor') value.actor.kind = 'local';
      else value.attentionFeatures = Array.from({ length: 65 }, (_, i) => 'feature-' + i);
    ...
    assert.equal(f.saved()?.phase, 'issued');
    f.intercept(undefined);
    assert.equal((await f.client().finish(input)).owner, 'synthetic-owner');
    assert.equal(f.calls.filter((call) => call.url.endsWith('/desktop/finish')).length, 1);
  });
