...
import type { AgentCallbacks, AgentRunBinding } from '@moor/host/agents/driver';
import {
  INTERACTION_VERSION,
  type QuestionAnswer,
  type QuestionRequest,
} from '@moor/protocol/interaction-protocol';
import type { SessionEvent } from '@moor/host/sessions/events';
...
async function fixture(
  ...
  const driver = createAcpDriver((_command, _args, options) => {
    child = spawn(
      process.execPath,
      [
        resolve('tests/fixtures/synthetic-acp-interactions.mjs'),
