...
import { DesktopGitHubSettings } from '../../apps/desktop/src/main/github-settings.cjs';

const entry = resolve('apps/host/src/main.ts');
function fixture(t: { after(fn: () => unknown): void }) {
  ...
  function desktop() {
    const child = fork(
        entry,
        [...args, '--desktop', '--server', '', '--public-dir', resolve('apps/web/public')],
        {
          env: environment,
          execArgv: ['--import', 'tsx'],
