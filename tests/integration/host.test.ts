...
import type { AgentDriver } from '@moor/host/agents/driver';
import { Store } from '@moor/gateway/accounts';
import { Flock, LoroDoc, delta, metas, mirror, putMeta, vv } from '@moor/session/model';
import type { AttentionContext, AttentionListQuery } from '@moor/protocol/attention';
import {
...
test('attention approval reads never approve; exact scoped retry responds only once', async (t) => {
  ...
  for (const busy of [f.host.executionManager.busy, f.host.forkManager.busy]) {
    busy.add(item.sessionId);
    await strict.rejects(f.host.attentionPermission(context, item.itemId, choice), /Git|Fork/);
...
test('attention continuation commits the new turn and original disposition atomically', async (t) => {
  ...
  for (const busy of [f.host.executionManager.busy, f.host.forkManager.busy]) {
    busy.add(item.sessionId);
    await strict.rejects(f.host.attentionContinue(context, item.itemId, continuation), /Git|Fork/);
