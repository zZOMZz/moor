...
const desktopMain = path.join(desktop, 'main');
...
const { CLIENT_SCHEME, CLIENT_PRIVILEGES, CLIENT_URL } = require(
  path.join(desktopMain, 'client-assets.cjs'),
);
const { prepareClientSession, createClientWindow } = require(
  path.join(desktopMain, 'client-window.cjs'),
);
const { DesktopSecureBridge } = require(path.join(desktopMain, 'secure-client.cjs'));
const { DesktopSecureAccount } = require(path.join(desktopMain, 'secure-account.cjs'));
const { createAttachmentSaver } = require(path.join(desktopMain, 'attachment-save.cjs'));
  ...
  app
    ...
    .then(async () => {
      ...
      window = createClientWindow({
        ...
        origin: fixture.origin,
        preloadPath: path.join(desktop, 'preload/secure-preload.cjs'),
        registry,
        invalidate: (contents) => {
