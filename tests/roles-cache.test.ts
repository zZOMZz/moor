...
const initial = {
  ...
  actor: { scope: 'synthetic-account', text: 'original' },
};
const replacement = {
  ...
  actor: { scope: 'synthetic-account', text: 'original + instructions' },
};
const entries = () =>
...
test('role draft transaction saves all five keys including actor together and rejects a competing page without partial writes', async () => {
...
test('a changed run selection or role or actor marker aborts role application without changing the plain draft', async () => {
  for (const key of ['run', 'marker', 'actor']) {
