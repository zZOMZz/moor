...
import { createWebPushTransport, validatePushSubscription } from './web-push';
...
async function main() {
  ...
  const { createApp } = await import('@moor/gateway/http');
  ...
  try {
    ...
    const activeApp = (app = createApp(store, {
      ...
      pushTransport: createWebPushTransport(),
