...
function renderAppearance(value) {
  if (!['dark', 'light', 'system'].includes(value)) value = 'system';
  document.documentElement.dataset.theme = value;
  document.querySelectorAll('input[name="appearance"]').forEach((input) => {
    input.checked = input.value === value;
  });
  $('appearance-status').textContent =
    value === 'system' ? '自动跟随系统的深色或浅色外观。' : '外观设置已保存。';
}
window.personal.onAppearance?.(renderAppearance);
$('appearance-options').onchange = async (event) => {
  const previous = document.documentElement.dataset.theme;
  $('appearance-options').disabled = true;
  try {
    renderAppearance(await window.personal.appearance(event.target.value));
  } catch (error) {
    renderAppearance(previous);
    $('appearance-status').textContent = error.message;
  } finally {
    $('appearance-options').disabled = false;
  }
};
let projects = [],
  bootstrapAgents = [];
