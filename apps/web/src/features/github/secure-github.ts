...
import { SecureScopedStorage, secureGitTarget } from '../../platform/secure-scoped-storage';
...
export type GithubSessionStorage<T> = {
  forTarget(
    target: T,
    current: () => void,
  ): Pick<import('../git/git-workspace').GitWorkspaceDependencies, 'read' | 'compareWrite'>;
...
export type GithubSessionDependencies<T> = {
  ...
  gitTarget(target: T): import('../git/git-workspace').GitTarget;
  request(
    target: T,
