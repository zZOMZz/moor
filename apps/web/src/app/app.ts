...
import { showSkillsControl, showSkillsPanel } from '../features/skills/skills-ui';
import {
  previewAnnotationKey,
  pendingPreviewMutationSchema,
  type PreviewAnnotationSubmission,
} from '../features/preview/project-preview';
import { GithubWriteController, githubWriteKey } from '../features/github/github-write';
import { showGithubWritePanel } from '../features/github/github-write-ui';
import { GITHUB_FEATURE } from '@moor/protocol/github-protocol';
import { GithubController, githubKey } from '../features/github/github';
import { showGithubControl, showGithubPanel } from '../features/github/github-ui';
import { GIT_WORKTREE_FEATURE } from '@moor/protocol/git-protocol';
import {
  GitWorkspaceController,
  gitWorkspaceKey,
  type GitTarget,
} from '../features/git/git-workspace';
import { showGitWorkspaceControl, showGitWorkspacePanel } from '../features/git/git-workspace-ui';
import {
...
import {
  ...
  sessionForkKey,
  type ForkTarget,
} from '../features/fork/session-fork';
import {
  showSessionForkControl,
  showSessionForkPanel,
  showForkOrigin,
} from '../features/fork/session-fork-ui';
import { NotificationController } from '../features/notifications/notifications';
import {
  notificationBrowser,
  browserNotificationReason,
  clearLocalNotifications,
  reconcileNotificationAccount,
} from '../features/notifications/notification-browser';
import { showNotificationPanel } from '../features/notifications/notification-ui';
import {
...
import {
  ...
  type QuestionItem,
  type SteerItem,
} from '../features/interactions/interactions';
import {
...
} from '../features/interactions/interaction-ui';
import { api as request, ApiError, type Identity } from '../platform/api';
import { SESSION_SEARCH_FEATURE, type SearchHit } from '@moor/protocol/search-protocol';
import {
  searchSessionContent,
  type SearchSession,
  type SessionSearchView,
} from '../features/sessions/session-search';
import {
  showSessionSearchControl,
  showSessionSearchPanel,
} from '../features/sessions/session-search-ui';
...
import { AttentionController, type AttentionRoute } from '../features/attention/attention';
import { AttentionWorkbench } from '../features/attention/attention-ui';
...
import type { Workspace, ProjectReplica } from '@moor/protocol/catalog';
import * as cache from '../platform/cache';
import { esc, renderItem, renderFileChanges } from '../components/content';
import {
...
async function appendSkillDraft() {
  ...
  try {
    ...
    const actor = attentionActor;
    const marker = actor ? { scope: draftActorScope(actor), text } : undefined;
    ...
    const writing = cache.write(draftKey + '/actor', marker);
    composerDraftWrites = writing;
    await writing;
    if (!current() || field.value !== original) return;
    field.value = text;
...
function renderSkills() {
  showSkillsControl(
    attentionVisible ? undefined : { disabled: !gitTarget(), onOpen: () => run(openSkills) },
  );
...
async function saveComposerDraft(value: string) {
  await persistComposerDraft(key('draft'), value);
}
...
function renderProjectControls() {
  showSessionSearchControl(
    !attentionVisible && sessionId && meta?.id === sessionId && replica
      ? {
          enabled:
...
function renderHistory() {
  ...
  const html =
    state.history
      .map(
        (turn) =>
          `<article class="turn ${turn.role}" data-turn-id="${esc(turn.id)}" data-search-turn="${esc(turn.id)}"><div class="turn-label">${turn.role === 'user' ? '你' : esc(meta?.agentType ?? 'Agent')} <time>${new Date(turn.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></div>${(
            turn.items ?? []
          )
            .map(
