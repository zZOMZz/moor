...
桌面启动优先打开随应用提供的本机工作区，中转在后台连接；现有远程界面使用菜单“我的所有电脑”。另有“加密工作区”入口，使用安装包内的可信页面，支持独立 Google 登录、设备配对、主机确认的副本、核心加密会话和权限审批。审批固定用户审阅的完整工具项、原回合、请求与选项，主机接收时再次核对；未报告 `permission-review-v1` 的旧主机不能接收这类审批及其原操作恢复。取消审批向原请求提交取消结果；封存待确认操作需由主机核查，不能撤回已接受的决定。启用步骤与尚需迁移的功能见[加密进展](docs/end-to-end-encryption.md#可信客户端与剩余接线)。本机页面更新仅清理可重新生成的界面缓存，保留草稿与待确认请求。页面加载失败或超时会提供手动重试，重试加载不会自动发送任务。
