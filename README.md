...
中转服务持久化组织元数据，不保存会话索引、会话正文、主机目录路径或 Agent 配置；目录和配置由在线主机提供。主机离线时仍能看到工作区及副本关系，但只能阅读访问端自己已经缓存的会话；浏览器缓存不构成备份。默认桌面、Web/PWA 和普通 CLI 使用 HTTPS/WSS，仍为明文 v3 转发，中转进程能够看到内容。显式启用的 [v4 加密主机与 secure CLI](docs/cli.md#显式加密连接)已提供端点间加密，桌面“加密工作区”已接通配对、核心会话、精确权限审批、项目文件与历史 diff、附件草稿和安全预览保存，以及 Skills 阅读引用与逐回合额外 MCP；GitHub、网页预览与组织管理界面、完整设备管理、可信 PWA、加密事件和通知仍待迁移，M6.2 尚未完成，范围见[加密进展](docs/end-to-end-encryption.md)。
...
桌面启动优先打开随应用提供的本机工作区，中转在后台连接；现有远程界面使用菜单“我的所有电脑”。另有“加密工作区”入口，使用安装包内的可信页面，支持独立 Google 登录、设备配对、主机确认的副本、核心加密会话、权限审批、文件与历史 diff、附件选择与预览保存、Skills 阅读引用及逐回合额外 MCP。审批固定用户审阅的完整工具项、原回合、请求与选项，主机接收时再次核对；未报告 `permission-review-v1` 的旧主机不能接收这类审批及其原操作恢复。取消审批向原请求提交取消结果；封存待确认操作需由主机核查，不能撤回已接受的决定。启用步骤与尚需迁移的功能见[加密进展](docs/end-to-end-encryption.md#可信客户端与剩余接线)。本机页面更新仅清理可重新生成的界面缓存，保留草稿与待确认请求。页面加载失败或超时会提供手动重试，重试加载不会自动发送任务。
