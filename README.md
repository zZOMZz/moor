...
M5.1 已接通 [Skills 发现与引用](docs/skills.md)：按会话实际执行目录发现项目 Skill，并查看本机明确登记的全局目录；完整说明和版本可加入可编辑草稿，随后普通发送。发现不代表原生 Agent 已启用，附带脚本和资源不自动读取。接下来继续角色与 Agent 配置、CLI 和受限多 Agent 协作；M6 仍按明确需求与验收条件启动。
