...
“协作任务”支持先审查并启用有限任务计划，再随父指令发送。父 Agent 可通过五个私有工具协调同电脑、同项目的子任务，每个子任务使用独立 Git worktree；任务数、并发、轮数和时限都有明确上限。父回合结束或授权失效时请求停止对应子回合，未知操作由用户核查原编号，工作目录另行手动清理。需要父 Agent 实际支持 HTTP MCP；目标完成仍由用户核验，真实 Agent 与设备专项待验收。详见[有限多 Agent 协作](docs/session-tasks.md)。
...
桌面启动优先打开随应用提供的本机工作区，中转在后台连接；现有远程界面使用菜单“我的所有电脑”。另有“加密工作区”入口，使用安装包内的可信页面，支持独立 Google 登录、设备配对、主机确认的副本、核心加密会话、权限审批、文件与历史 diff、附件选择与预览保存、Skills 阅读引用、逐回合额外 MCP、GitHub 读取与审阅写入、网页预览和标注，以及 Git/worktree 与原生 Fork。审批固定用户审阅的完整工具项、原回合、请求与选项，主机接收时再次核对；未报告 `permission-review-v1` 的旧主机不能接收这类审批及其原操作恢复。取消审批向原请求提交取消结果；封存待确认操作需由主机核查，不能撤回已接受的决定。启用步骤与尚需迁移的功能见[加密进展](docs/end-to-end-encryption.md#可信客户端与剩余接线)。本机页面更新仅清理可重新生成的界面缓存，保留草稿与待确认请求。页面加载失败或超时会提供手动重试，重试加载不会自动发送任务。
...
“加密工作区”的 Skills 可查看项目与主机已授权全局来源，重新核对完整说明后加入原会话草稿。“额外 MCP”只读取名称、说明和版本等安全目录，保存最多 8 个选择，发送前重新核对原版本。选择与原指令一起记账，结果未知不重放，主机确认只清除此份审阅，后续回合不继承；读取和存草稿都不启动服务。选中 MCP 的加密发送还要求主机报告 `secure-turn-authority-v1`，原连接或执行身份失效会撤销对应回合授权。具体范围见[Skills](docs/skills.md#桌面加密工作区)及[MCP](docs/mcp.md#桌面加密工作区)，完整多设备验收仍待完成。
