...
M5.1 已接通 [Skills 发现与引用](docs/skills.md)：按会话实际执行目录发现项目 Skill，并查看本机明确登记的全局目录；完整说明和版本可加入可编辑草稿，随后普通发送。发现不代表原生 Agent 已启用，附带脚本和资源不自动读取。M5.2 已完成[会话固定 Agent 版本](docs/runtime.md#会话固定-agent-版本)的基础，配置变化不会追溯替换已有会话的启动参数。下一批继续角色预设和本机自定义 ACP 配置，再推进 CLI 与受限多 Agent 协作；M6 仍按明确需求与验收条件启动。
