...
- **会话导航**：以工作区为入口，汇总工作区内各台电脑的会话，按逻辑项目筛选，按会话标题、项目或电脑名称搜索；搜索暂不覆盖正文。会话支持重命名、置顶、归档和恢复。重新打开页面时恢复原执行副本、会话和本机草稿；手机使用可收起的会话抽屉。
...
- **输出阅读**：支持常用 Markdown、代码块复制、终端命令与输出、退出码，以及 Agent 工具返回的文件修改前后对照。完整回合和会话的文件变更采集尚未实现；界面的变更统计渲染不代表主机已经采集了统计数据。实时更新时保留已展开的工具记录。Agent 输出中的 HTML 不执行，也不自动加载远程图片。
...
Agent 返回的嵌入文件可从历史卡片预览或下载；PNG/JPEG/GIF/WebP、纯文本和 Markdown 支持预览，其他文件提供下载。普通资源链接不会触发网络抓取或本机路径读取。项目文件树、完整 diff 与正文搜索继续按[功能完善计划](docs/roadmap.md)推进，附件预览不代表完整文件浏览器已完成。详细范围见[文件与内容协议](docs/content.md)。
...
| 位置           | 持久化内容                                                                       |
| -------------- | -------------------------------------------------------------------------------- |
| 执行电脑       | 项目副本、Agent 配置、完整会话、附件字节与引用、请求去重记录；本机模式的组织设置 |
| 中转服务       | 账号、登录凭据哈希、设备绑定与撤销、工作区/逻辑项目/副本映射                     |
| 手机和其他电脑 | 按需获取的会话与附件版本缓存、自己的文本和附件草稿、待确认请求                   |
