...
GitHub 面板可读取已登记仓库的分支、Issue、PR、会话评论和指定提交的 CI，并保存本地会话关联。凭据只在执行电脑的连接设置或本机 CLI 中管理，配置目录必须位于项目之外。浏览内容不会自动提交给 Agent；明确加入草稿后仍需手动发送。本批不提供 GitHub 外部写入，详见[GitHub 仓库与会话上下文](docs/github.md)。
...
| 位置           | 持久化内容                                                                                                                               |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| 执行电脑       | 项目副本、Agent 配置、完整会话、附件字节与引用、回合文件基线、可重建正文索引、请求去重记录；本机组织设置、私有 GitHub 凭据和最小会话关联 |
...
GitHub token 不进入共享文档或中转。GitHub 正文仅供在线读取，浏览器不持久缓存；用户明确加入草稿的文本适用普通草稿与会话保存规则，随后撤销 GitHub 授权不会删除已经复制的内容。
...
M4.3 已接通 GitHub 本机配置、仓库与 PR/CI 读取及会话关联，四项仓库检查与 612 项自动测试通过，并完成合成浏览器检查。定向测试只使用合成 token、项目和 HTTP 响应，没有调用真实 GitHub API；真实账号授权及设备体验仍待专项验收。配置、读取限制与撤权恢复见[GitHub 文档](docs/github.md)。M4.4 外部写入、后续网页预览、M5 和按需求启动的 M6 尚未完成。
