...
中转服务持久化组织元数据，不保存会话索引、会话正文、主机目录路径或 Agent 配置；目录和配置由在线主机提供。主机离线时仍能看到工作区及副本关系，但只能阅读访问端自己已经缓存的会话；浏览器缓存不构成备份。默认桌面、Web/PWA 和普通 CLI 使用 HTTPS/WSS，仍为明文 v3 转发，中转进程能够看到内容。显式启用的 [v4 加密主机与 secure CLI](docs/cli.md#显式加密连接)已提供端点间加密，桌面“加密工作区”已接通配对与核心会话；富内容界面、可信 PWA、通知和完整设备管理仍待完成，范围见[加密进展](docs/end-to-end-encryption.md)。
...
桌面启动优先打开随应用提供的本机工作区，中转在后台连接；现有远程界面使用菜单“我的所有电脑”。另有“加密工作区”入口，使用安装包内的可信页面，支持独立 Google 登录、设备配对、主机确认的副本和核心加密会话；启用步骤与尚需迁移的功能见[加密进展](docs/end-to-end-encryption.md#可信客户端与剩余接线)。本机页面更新仅清理可重新生成的界面缓存，保留草稿与待确认请求。页面加载失败或超时会提供手动重试，重试加载不会自动发送任务。
