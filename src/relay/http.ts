...
export function createApp(
  ...
  const bridges = new Map<string, { socket: WebSocket; ready: boolean; workspaces: any[] }>();
  const viewers = new Map<
    WebSocket,
    {
      owner: string;
      secret: string;
      watch?: { deviceId: string; workspaceId: string; sessionId: string };
    }
  >();
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (path === '/api/pair' && req.method === 'POST')
        return json(res, 200, { code: store.pair(owner!), expiresIn: 300 });
