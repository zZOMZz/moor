...
void main() {
  ...
  vec3 deep = mix(vec3(.72), vec3(.07), dark);
  vec3 shallow = mix(vec3(.88), vec3(.17), dark);
  vec3 pearl = mix(vec3(.97), vec3(.27), dark);
  ...
  color += vec3(.18) * ribbon * mix(.35, .17, dark);
  color += vec3(.32) * caustic * mix(.22, .10, dark);
